Shader "Linear Lighting/Effects/GlowAdditiveTwoColor" {
	Properties {
		...
		SubShader {
			Pass {
				...
				fixed4 frag (v2f i) : COLOR
				{
					...
					fixed4 tex = LLDecodeTex( tex2D(_MainTex, i.texcoord) );
					fixed4 col = (LLDecodeGamma( _TintColor ) * tex.g * _TintStrength + tex.r * LLDecodeGamma( _CoreColor ) * _CoreStrength  - _CutOutLightCore); 
					
					return LLEncodeGamma( i.color * clamp(col, 0, 255) );
				}
